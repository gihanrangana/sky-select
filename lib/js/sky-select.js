export default class SkySelect {
    constructor(element) {
    ...
    selectValue(value) {
        ...
        this.optionsElement.querySelector(
            `[data-value="${prevSelectedOption.value}"]`
        ).classList.remove('selected')

        const newElement = this.optionsElement.querySelector(
...
function setupCustomElement(select) {
    ...
    select.customElement.classList.add('sky-select--container');
    select.customElement.tabIndex = 0;

    select.labelElement.classList.add('sky-select--value');
    select.labelElement.innerText = select.selectedOption.label;
    select.customElement.append(select.labelElement);

    select.optionsElement.classList.add('sky-select--options');

    select.options.forEach((option) => {
        const optionElement = document.createElement('li');
        optionElement.classList.add('sky-select--option');
